...
import 'package:plant_social/features/timelapse/presentation/screens/timelapse_session_detail_screen.dart';
...
class TimelapseTrackingWidget extends ConsumerWidget {
  ...
  Widget _buildSessionsOverview(ThemeData theme, List<TimelapseSession> sessions) {
    final totalPhotos = sessions.fold<int>(0, (sum, session) => sum + session.currentMetrics.totalPhotos);
  ...
  Widget _buildRecentProgress(ThemeData theme, List<TimelapseSession> sessions) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        ...recentPhotos.take(3).map((photo) => _buildProgressItem(theme, photo)),
      ],
    );
  }
        ...
        if (activeSessions.isEmpty)
          ...
          Row(
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: () => _capturePhoto(context, ref, activeSessions.first),
  ...
  Widget _buildGeneralTimelapseCard(BuildContext context, ThemeData theme, WidgetRef ref) {
    return Card(
      elevation: 4,
      child: Padding(
        padding: const EdgeInsets.all(16),
  ...
  void _startNewSession(BuildContext context, WidgetRef ref) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Session setup wizard coming soon!'),
      ),
    );
  }

  void _capturePhoto(BuildContext context, WidgetRef ref, TimelapseSession session) {
    // Trigger manual photo capture
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Capturing photo for ${session.sessionName}...'),
      ),
    );
  }
  ...
  void _viewSession(BuildContext context, TimelapseSession session) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => TimelapseSessionDetailScreen(sessionId: session.sessionId),
      ),
    );
  }
  ...
  void _showAllSessions(BuildContext context) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('All sessions view coming soon!'),
      ),
    );
  }
